...
class Customers_acc : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val exitAcc_b = findViewById<Button>(R.id.exitAcc_b)
        exitAcc_b.setOnClickListener {
            auth.signOut()
        ...
        deleteAcc_b.setOnClickListener() {
            ...
            if (userId != null) {
                db.collection("users").document(userId)
                        ...
                        user.delete()
                            .addOnCompleteListener { task ->
                                if (task.isSuccessful) {
                                    Log.d(
                                        "Auth",
                                        "Акаунт користувача видалено з Firebase Authentication"
                                    )
                                    Toast.makeText(
                                        this,
                                    ...
                                    Log.w(
                                        "Auth",
                                        "Помилка при видаленні акаунта: ${task.exception?.message}"
                                    )
                                    Toast.makeText(
                                        this,
                    ...
                    .addOnFailureListener { e ->
                        Log.w("Firestore", "Помилка при видаленні документа: ${e.message}", e)
                        Toast.makeText(
                            this,
                            "Помилка при видаленні даних: ${e.message}",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
            } else {
